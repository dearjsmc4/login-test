      ...
      <section>
        <form [formGroup]="signupForm" (ngSubmit)="onSubmit()">
          <div class="form-group" formGroupName="emailGroup">
            ...
            <p class="error" *ngIf="emailId.errors?.required && emailId.touched">
              이메일을 입력해 주세요.</p>
            <p class="error" *ngIf="emailDomain.errors?.pattern && emailDomain.touched">
              이메일 형식이 올바르지 않습니다.</p>
          </div>
          ...
          <div class="form-group">
            ...
            <p class="error" *ngIf="nickname.errors?.required && nickname.touched">
              별명을 입력해주세요.</p>
      ...
      <section class="sign-up-policy">
        ...
        <div class="policy-form">
          <div class="policy age">
            <label class="policy-over-14" for="age-check">
            만 14세 이상입니다.
            </label>
            <label for="age-check" class="bold check-label">
              <input type="checkbox" id="age-check" class="check">
              동의<span class="text-red">(필수)</span>
            </label>
          </div>
          <div class="policy use">
            <a href="#" class="policy-link bold">
            이용약관
            </a>
            <label for="use-check" class="bold check-label">
              <input type="checkbox" id="use-check" class="check">
              동의<span class="text-red">(필수)</span>
            </label>
          </div>
          <div class="policy personal-info">
            <a a href="#" class="policy-link bold">
            개인정보취급방침
            </a>
            <label for="personal-info-check" class="bold check-label">
              <input type="checkbox" id="personal-info-check" class="check">
              동의<span class="text-red">(필수)</span>
            </label>
          </div>
          <div class="policy event">
            <label class="policy-event" for="event-check">
            이벤트 등 프로모션 알림 메일 수신
            </label>
            <label for="event-check" class="bold check-label">
              <input type="checkbox" id="event-check" class="check">
              동의<span class="text-grey">(선택)</span>
            </label>
          </div>
        </div>
        <label for="all-check" class="bold agree-all">
          <input type="checkbox" id="all-check" class="check">
          전체 동의
        </label>
      </section>
      <button type="submit" class="submit" (click)="onSubmit()">회원가입하기</button>
      <p class="has-account">이미 아이디가 있으신가요? <a href="#" class="bold login-link">로그인</a></p>
      </div>
    </main>
  </div>
  `,
...
export class SignUpComponent implements OnInit {
  ...
  onSubmit(){
    ...
    this.signUp.create({ email, password, nickname });
    // .subscribe();
  }
